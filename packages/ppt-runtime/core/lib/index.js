import { bundledProjectTemplates, bundledProjectTemplate, bundledProjectSource, copyBundledProject } from "./bundled-template-projects.js";
...
import { PersonalTemplateLibrary } from "./personal-templates.js";
/**
...
const DSH_PPT_PROMPT = [
	...
	"For a selected personal template or a built-in template with an editable project, use ppt_template_create_project to copy its editable pages and assets into the active workspace, then inspect and adapt that copy. Preserve reviewed company branding and replace sample facts with current task material.",
...
function clearAutomaticPptContext(agent, staleOnly = false) {
	for (const seq of [...agent.session.surface.nodes]) {
		...
		if (staleOnly && source.plugin !== "kimi-ppt-skill" && source.plugin !== "kimi-ppt-composer" && (source.plugin !== SKILL_PLUGIN || event.data.content.some(part => part.type === "text" && part.text.includes("DSH-PPT-AUTHORING-20260914-IMAGE-TEMPLATES-V1")))) continue;
	...
	ctx.tools.register(defineTool({
		...
		async execute(args, exec) {
			...
			if (template.origin === "personal" || bundledProjectTemplate(template.id)) {
				const source = bundledProjectTemplate(template.id) ? await bundledProjectSource(template.id) : await service.store.personalTemplates.projectSource(template.id);
				...
				return Promise.all(template.pageIndex.filter(p => numbers === void 0 || numbers.includes(p.slideNumber)).map(async page => {
					const pageText = await readFile(path.join(source, page.file), "utf8");
