...
使用 DSH 自有基础 Skill 和 office_template 返回的 authoring 执行 office_build。保持输入顺序与修订哈希。Excel 生成后调用 office_recalculate，并使用内置 Office 文档预览核对所有展示页。当前案例通过 DSH 工具直接生成；模型自主规划的结果需另行验收。
