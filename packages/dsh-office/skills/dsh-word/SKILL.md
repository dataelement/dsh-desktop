...
宿主内置业务 Skill，名称与用途见当前会话的 Skill 目录。遇到匹配的研究报告、营销方案、会议纪要、工作汇报等任务，先调用 `skill(name="目录中的准确名称")` 加载对应业务方法，再用本 Skill 落实 Word 输出。按需使用 `office_skill_read` 读取其参考资料，使用 `office_skill_prepare` 准备脚本与资产。简单文字修改直接使用本基础流程。用户从案例预览点击“做同款”后，宿主会给出准确的案例编号与修订版本；先调用对应的 `office_template`，再以用户描述和材料替换案例事实。
