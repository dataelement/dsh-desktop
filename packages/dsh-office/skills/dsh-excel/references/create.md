...
`office_build(language="python", source, inputs, output_file)` 使用已安装的 Python 和固定版本 `openpyxl==3.1.5`。脚本得到 `office["inputs"]`、`office["output"]`、`office["directory"]`。CSV/JSON 等文本材料先用标准 `read` 工具读取；DOCX/XLSX 材料用 `office_inspect`。需要由脚本访问的工作区文件按 `office_build` 当前工具架构声明为输入。目录中只有输入副本和临时产物，网络关闭。Python 标准库可用于处理 CSV/JSON 等材料；openpyxl 用于工作簿、公式、格式、图表和条件格式。
...
生成后使用 `office_recalculate`，以上例子的独立检查可以是 `[{"sheet":"采购","cell":"D4","expected":760}]`。随后在 DSH Desktop 内置 Office 文档预览中检查页面。`openpyxl` 保存只生成文件，计算结果由后续引擎提供。最终 XLSX 以及用户要求的 PDF 使用基础 `present` 工具交付。
