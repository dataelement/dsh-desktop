...
function parseModelEntry(entry, seen) {
  if (!isObject(entry) || !isObject(entry.capabilities)) return undefined
  if (typeof entry.id !== 'string' || entry.id.length === 0 || entry.id.length > 512) return undefined
  ...
  seen.add(entry.id)
  const usage = isObject(entry.usage) ? parseUsageLenient(entry.usage) : undefined
  return {
    id: entry.id,
    object: typeof entry.object === 'string' && entry.object.length > 0 ? entry.object : 'model',
    created: Number.isSafeInteger(entry.created) ? entry.created : 0,
    owned_by: typeof entry.owned_by === 'string' && entry.owned_by.length > 0 ? entry.owned_by : 'bisheng',
    display_name: typeof entry.display_name === 'string' && entry.display_name.length > 0 ? entry.display_name : entry.id,
    capabilities: {
    ...
    ...(usage && usage.source !== 'unavailable' ? { usage } : {})
  }
}
...
export function parseModels(value) {
  ...
  return value.data.map((entry) => {
    ...
    if (!parsed) throw new Error('BiSheng models response contains an invalid model.')
    return parsed
  })
}

export function parseModelsLenient(value) {
  if (!isObject(value) || value.object !== 'list' || !Array.isArray(value.data)) {
    return []
  }
  const seen = new Set()
...
function unwrapUsage(value) {
  ...
  if (isObject(value.usage)) return value.usage
  if (isObject(value.data) && !Array.isArray(value.data)) return value.data
  if (isObject(value.quota)) return value.quota
  return value
}
...
export function parseUsageLenient(value) {
  const raw = unwrapUsage(value)
  if (!isObject(raw)) return unavailableUsage()
  const used = optionalInteger(raw.used ?? raw.used_tokens ?? raw.consumed)
  const limit = optionalInteger(raw.limit ?? raw.total ?? raw.quota ?? raw.max)
  const remaining = optionalInteger(raw.remaining ?? raw.left)
  const source = ['live', 'persisted', 'unavailable'].includes(raw.source)
    ? raw.source
    : used === null && limit === null ? 'unavailable' : 'live'
  const quotaState = ['available', 'exhausted', 'unavailable'].includes(raw.quota_state)
    ? raw.quota_state
    : limit === 0 || (used !== null && limit !== null && used >= limit)
      ? 'exhausted'
      : used === null && limit === null ? 'unavailable' : 'available'
  return {
    ...(typeof raw.month === 'string' ? { month: raw.month } : {}),
    ...(typeof raw.billing_timezone === 'string' ? { billing_timezone: raw.billing_timezone } : {}),
    ...(optionalTimestamp(raw.period_start) ? { period_start: optionalTimestamp(raw.period_start) } : {}),
    ...(optionalTimestamp(raw.reset_at) ? { reset_at: optionalTimestamp(raw.reset_at) } : {}),
    used,
    limit,
    remaining,
    source,
    as_of: optionalTimestamp(raw.as_of),
    quota_state: quotaState
  }
}
...
export async function requestJson(baseValue, path, options = {}) {
  const base = normalizeEnterpriseServerUrl(baseValue, {
    ...
    allowInsecurePrivateHttp: options.allowInsecurePrivateHttp === true
  })
