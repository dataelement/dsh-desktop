...
### 启动失败的结构化归因（2026-09-10）

Host 启动失败现在优先使用加载现场的数据：`loadProfile` 在读取 bundle patch 时给 loader entry 保留根包名、实际加载版本和包目录；嵌套 group/include 的子节点通过父 entry 继承来源。loader 在导入、初始化及激活失败时，将阶段、失败组件、entry 链和根包来源附在异常上。准备 bundle 时的解析或文件错误也使用同一契约。

`harness-node-entry.mjs` 遍历 `cause`/`AggregateError`，保留最内层失败记录，通过带版本号的单行 JSON 上报给 Desktop，再输出原有可读错误。报告不包含插件 config。Desktop 将报告单独保存在当前启动的 snapshot 中，避免日志环形缓冲截断导致归因丢失；每次启动清空旧报告。

恢复流程用明确的根包身份校验当前 Profile 的第三方启用列表，再复用市场兼容版本检查、升级、卸载和安全模式。结构化报告存在但根来源未知、受保护或已移除时，不混入日志推测的卸载候选。旧版 Harness 和前端错误没有该报告时，保留原有日志归因路径。

这仍是失败后恢复：不会吞掉异常继续运行半初始化的树，不会自动卸载或改写 `desired`；升级和卸载前停止 Harness。本机制不等于执行前验证任意 API 方法，也不覆盖脱离初始化任务的定时器、后台任务或所有前端异步异常。加载失败的归属用于确定恢复对象，不代表该插件一定是兼容性问题的根因。
...
恢复页面会检查市场中的插件兼容更新；找到可用版本时，用户可以升级对应插件。已识别有问题的插件若没有匹配版本，也会提供更新到 latest 的入口，并说明兼容性未确认。安全模式支持一键升级当前检查结果中有更新的插件，也可以卸载选中的问题插件，再尝试正常启动。没有可用更新或无法连接市场时，不应把它当成“插件一定兼容”或“已是最新版”。
...
市场检查读取完整版本元数据，在已安装版本之后、`dist-tags.latest` 以内按语义版本从高到低选择候选，逐个检查 DSH peer、`engines.dsh`、`dsh.minVersion` 和已移除依赖规则。最新版要求更高 DSH 时，优先选择中间的兼容版本。匹配候选排除弃用版本；正式版安装默认排除预发布版本，已安装预发布版则允许预发布候选。如果已识别加载异常或不兼容的插件没有匹配候选，且 latest 高于已安装版本，则提供用户主动尝试 latest 的入口，不再以兼容性或发布筛选条件阻止该兜底候选，并提示不保证兼容、保留声明冲突原因。两条路径都不会降级或重装当前版本；无法读取市场信息或确定已安装版本时不提供升级。`latestVersion` 保留市场 latest，`upgradeVersion` 指定实际安装目标。声明未发现冲突不等于启动错误已经修复，缺少兼容声明时尤其需要升级后验证。

安全模式和启动恢复页都提供社区入口：悬停“微信群”显示二维码，点击 Discord 打开社区链接。反馈启动问题时，建议附上安装包版本、来源或 SHA-256，以及去除密钥等敏感信息后的技术详情和 `harness.log`。

恢复页对所有已定位的阻塞插件分别检查更新，一个插件查询失败不影响其他插件。恢复页和安全模式通过 Electron 网络接口查询市场，使用应用的系统网络/代理路径；只缓存成功的版本信息，失败后重试立即重新请求。所有查询都失败或无法确定版本时，主按钮显示“重新检查更新”，不会显示零项自动处理；错误提示保留具体的网络或 HTTP 失败原因。每项显示独立的版本建议和升级/卸载按钮，操作按完整包名定位；主按钮为“一键自动处理”，显示计划升级和卸载的数量：有候选版本的插件升级，已确认无更新可用或已尝试目标版本仍阻塞的插件卸载。停止 Harness 后依次执行两类操作，保留部分成功结果，最后统一检查和启动，不在每个插件之间重启。查询失败或版本未知的插件跳过自动处理，保留逐项操作；升级失败不自动转为卸载，卸载未完成时不重启正常模式。

成功安装后，该插件的旧启动失败记录失效；其他插件仍阻塞时，不会仅凭旧记录继续推荐它升级到 latest。只有当前检查确认该插件仍有阻塞项，或新的启动尝试再次报告该插件异常时，才重新纳入处理。真正发起新启动时同时清除旧前端错误上下文；启动前维护被拦截不算新的加载失败。安装失败仍可重试。

如果已定位的问题插件已是 latest 或更高版本，仍阻挡启动，则提示卸载该插件并继续检测，不提供降级或重复安装。恢复页提供卸载操作，用户点击后停止 Harness，复用插件卸载和重新检测流程；安全模式也显示卸载建议。
