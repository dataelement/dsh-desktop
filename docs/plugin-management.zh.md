...
恢复页对所有已定位的阻塞插件分别检查更新，一个插件查询失败不影响其他插件。每项显示独立的版本建议和升级/卸载按钮，操作按完整包名定位；主按钮为“一键自动处理”，显示计划升级和卸载的数量：有候选版本的插件升级，已确认无更新可用或已尝试目标版本仍阻塞的插件卸载。停止 Harness 后依次执行两类操作，保留部分成功结果，最后统一检查和启动，不在每个插件之间重启。查询失败或版本未知的插件跳过自动处理，保留逐项操作；升级失败不自动转为卸载，卸载未完成时不重启正常模式。
