...
正常启动还会独立检查已启用 dshmarket 的实际版本。恢复锁、待确认卸载和迁移延迟期间跳过此升级；否则在 Harness 停止、generation 投影完成后检查，即使 Profile 早已完成迁移也执行。实际版本低于 Desktop 验证基线（当前 `1.45.1`），或已启用市场的文件缺失时，使用市场配置的 registry 将精确基线版本安装到独立 generation，再投影并重新读取实际版本。安装失败或复核失败进入安全恢复，不把仅修改依赖声明视为升级成功。已安装或已暂存并投影的更高版本不降级；已卸载或禁用的市场不自动装回。这不触发整个 Profile 的依赖重装。

市场检查读取完整版本元数据，在已安装版本之后、`dist-tags.latest` 以内按语义版本从高到低选择候选，逐个检查 DSH peer、`engines.dsh`、`dsh.minVersion` 和已移除依赖规则。最新版要求更高 DSH 时，可以选择中间的兼容版本；不会降级或重装当前版本，也不会选择弃用版本。正式版安装默认排除预发布版本，已安装预发布版则允许预发布候选。`latestVersion` 保留市场 latest，`upgradeVersion` 指定实际安装目标。声明未发现冲突不等于启动错误已经修复，缺少兼容声明时尤其需要升级后验证。
