...
3. 将展开状态侧边栏的品牌行标记为拖拽区。品牌按钮因 `flex: 1` 会占满 Logo 到折叠按钮之间的空白，因此把该按钮的空白部分也设为拖拽区，再仅对 `[data-dsh-sidebar-brand-identity]` 可见品牌内容恢复 `no-drag`；Logo 仍可点击新建会话，折叠按钮仍可点击。
4. 为按钮、链接、表单控件、`[role="button"]`、`[role="tab"]` 和 `[data-dockkit-strip]` 显式设置 `-webkit-app-region: no-drag !important`，从标题栏拖拽区中保留完整交互命中范围。
5. 将应用根节点放在顶部 6px 拖拽层之上，避免普通控件被透明元素覆盖。
6. 带 `[data-dockkit-strip-chrome]` 的右侧面板 Tab 条预留 Windows 原生标题栏按钮及应用菜单宽度，防止面板按钮与原生最小化、最大化、关闭按钮重叠。
...
- [ ] 在左上角 Logo 与侧边栏折叠按钮之间的空白处按住鼠标可以拖动窗口。
- [ ] 点击 Logo 仍能新建会话，点击侧边栏折叠按钮仍能展开或收起侧边栏。
