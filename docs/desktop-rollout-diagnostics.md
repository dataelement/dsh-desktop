...
The installation ID is a random UUID at `<userData>/desktop-service/installation.json`. Queue and session marker live alongside it. Maximum pending reports: 50 (oldest evicted); each report is removed from the queue before its single upload attempt, with a 5-second timeout. Failed or interrupted uploads are discarded, without retries or periodic polling. Reports captured before networking is available (including fatal crashes) get their first attempt at the next startup. Long lines are capped at 2000 characters, file reads at 1 MiB; truncation or missing files are marked.
