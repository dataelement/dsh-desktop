...
The installation ID is a random UUID at `<userData>/desktop-service/installation.json`. Queue and session marker live alongside it. Maximum pending reports: 50 (oldest evicted); requests time out after 5 seconds and retry at startup and every 30 seconds. A matching server acknowledgment is required before deleting a queued event. Long lines are capped at 2000 characters, file reads at 1 MiB; truncation or missing files are marked.
