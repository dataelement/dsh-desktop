...
describe('BiSheng compatible client API mock', () => {
  ...
  it('skips invalid models without failing the whole list', () => {
    expect(parseModelsLenient({
      object: 'list',
      data: [
        {
          id: 'bisheng:42',
        ...
        { id: '', capabilities: { streaming: true, tools: true, reasoning_content: false } },
        { display_name: 'broken' }
      ]
    })).toEqual([
      {
        ...
        owned_by: 'bisheng',
        display_name: 'Good',
        capabilities: { streaming: true, tools: true, vision: false, reasoning_content: false }
      }
    ])
  })
  ...
  it('reads partial usage payloads without failing the catalog', () => {
    expect(parseUsageLenient({ used: 128, limit: 1000 })).toMatchObject({
      used: 128,
      limit: 1000,
      source: 'live',
      quota_state: 'available'
    })
    expect(parseUsageLenient({ data: { used: '40', total: 80 } })).toMatchObject({
      used: 40,
      limit: 80,
      source: 'live'
    })
    expect(parseUsageLenient({})).toMatchObject({
      used: null,
      limit: null,
      source: 'unavailable',
      quota_state: 'unavailable'
    })
    expect(parseUsageLenient(undefined).source).toBe('unavailable')
    expect(parseModelsLenient({
      object: 'list',
      data: [{
        id: 'qwen-max',
        capabilities: { streaming: true, tools: true, reasoning_content: false },
        usage: { used: 12, limit: 99 }
      }]
    })[0]?.usage).toMatchObject({ used: 12, limit: 99, source: 'live' })
  })
})
