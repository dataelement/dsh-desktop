...
describe('Windows titlebar menu', () => {
  ...
  it('keeps the Windows app full-height while making only empty titlebar surfaces draggable', async () => {
    const main = await readFile('src/main/index.ts', 'utf8')
    ...
    expect(preload).toContain('> div:has([data-dsh-sidebar-brand-identity])')
    expect(preload).toContain('> button:has([data-dsh-sidebar-brand-identity])')
    ...
    expect(preload.match(/-webkit-app-region: drag/g)).toHaveLength(4)
