...
describe('durable model preference', () => {
  ...
  it('waits for the saved enterprise login to restore before publishing a startup catalog', async () => {
    let release
    const ready = new Promise(resolve => { release = resolve })
    const listProviders = vi.fn(() => [])
    const loading = buildModelCatalog({ get: () => ({ ready }), agentDefaultModel: { currentSelection: () => enterprise, fallbackSelection: () => previous }, llm: { listProviders } })
    await settle()
