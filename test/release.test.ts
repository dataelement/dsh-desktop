...
describe('GitHub release contract', () => {
  ...
  it('builds and publishes every supported platform', async () => {
    ...
    expect(workflow).toContain("Invoke-PluginRpc '/dsh-office' 'mode'")
    expect(workflow).toContain("Invoke-PluginRpc '/dsh-ppt' 'presentation/mode'")
    expect(workflow).toContain('Packaged Office and PPT RPC smoke test passed.')
