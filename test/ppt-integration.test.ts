...
describe('DSH PPT built-in plugin', () => {
  ...
  it('places format actions after the agent preset above the input and keeps the catalog below it', async () => {
    ...
    const modeActions = client.indexOf('children: renderSlot("conversation.hero.modeActions", zone)')
    ...
    expect(modeActions).toBeLessThan(owner)
    expect(catalog).toBeGreaterThan(input)
    expect(client).toContain('"data-office-mode-row": ""')
    const rootRegistration = client.indexOf('const registerConversationRoot = () => slots.register(')
    const rootChildren = client.slice(rootRegistration, client.indexOf('inject: (sessionId)', rootRegistration))
    expect(rootChildren).toContain('"conversation.hero.modeActions":')
    const registration = client.indexOf('const registerComposerBar = () => slots.register(')
    const children = client.slice(registration, client.indexOf('inject: (sessionId)', registration))
    const component = client.slice(client.indexOf('const InputBar = '), client.indexOf('//#endregion', client.indexOf('const InputBar = ')))
    for (const [, slot] of component.matchAll(/renderSlot\("([^"]+)"/gu)) {
      expect(children, `InputBar must own the slot it renders: ${slot}`).toContain(`"${slot}":`)
    }
  })
  ...
  it('integrates hero mode actions with the adjacent agent-preset control style', async () => {
    ...
    expect(patch).toMatch(/\.[A-Za-z0-9_-]+_heroModeCluster\{padding:0 8px;width:max-content/)
