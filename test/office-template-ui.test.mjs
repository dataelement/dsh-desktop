...
it('selects an example only from the full preview and exposes the reference in the composer', async () => {
  ...
  const selected = container.querySelector('.wbo-selected')
  expect(selected.getAttribute('aria-label')).toContain('equity-research')
  expect(selected.querySelectorAll('img')).toHaveLength(1)
  expect(selected.querySelector('.wbo-selected-copy')).toBeNull()
  expect(selected.querySelector('.wbo-selected-preview')).not.toBeNull()
  expect(selected.querySelector('.wbo-selected-remove')).not.toBeNull()
