...
describe('DSH Desktop sidebar branding', () => {
  ...
  it('keeps the phone entry aligned at the right edge of the settings row', async () => {
    ...
    expect(preload).toContain("const VERSION_LABEL_ID = 'dsh-desktop-version-label'")
    expect(preload).toContain('settingsArea.insertBefore(versionLabel, mobileButton ?? null)')
    expect(preload).toContain('const version = currentStatus?.currentVersion')
    expect(preload).toContain('[data-dsh-sidebar-wide="false"] #${VERSION_LABEL_ID}')
