	...
	factory: (require) => {
		...
		*/
		...
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=" + JSON.stringify(tagId$17) + "]") === null) {
			...
			tag.textContent = css$17 + ".uV2eYG_primary{background:#0f1115}.uV2eYG_primary:hover:not(:disabled){background:#23262b}body[data-ds-dark-theme] .uV2eYG_primary{background:#f5f5f5;color:#202124}body[data-ds-dark-theme] .uV2eYG_primary:hover:not(:disabled){background:#fff}";
			document.head.appendChild(tag);
		}
		...
		* has not exposed a visible box yet. */
		...
		function isExecutionDetailNode(node, closingSeqByTurn) {
		...
		function compactConversationFlow(order, nodeStore, runningTurn) {
			...
			for (const key of order) {
				...
				if (turn !== null && typeof seq === "number") closingSeqByTurn.set(turn, seq);
			}
			const entries = [];
			...
			for (const key of order) {
				const node = nodeStore.get(key);
				if (!isExecutionDetailNode(node, closingSeqByTurn)) {
			...
			for (const entry of entries) {
				...
				if ((segmentCounts.get(String(entry.turn)) ?? 0) > 1) entry.preserveProgress = true;
			}
			return entries;
		}
		...
		*/
		...
		function canonicalResearchCanvasArtifactNode(value) {
			...
			if (id === null || kind === null || messageId === null || title === null || excerpt === null || !Number.isFinite(value.x) || !Number.isFinite(value.y)) return null;
		...
		function parseResearchCanvasArtifactNodes(raw) {
			...
			try {
				...
				for (const item of value) {
					...
					const source = node === null ? null : `${node.kind}:${node.messageId}`;
		...
		function parseResearchArtifactDrag(raw) {
			if (typeof raw !== "string" || raw.length > RESEARCH_CANVAS_PERSISTED_FILES_RAW_LIMIT) return null;
			try {
				const value = JSON.parse(raw);
				if (typeof value !== "object" || value === null) return null;
				...
				const title = boundedResearchArtifactString(value.title, RESEARCH_ARTIFACT_MAX_TITLE);
				const excerpt = boundedResearchArtifactString(value.excerpt, RESEARCH_ARTIFACT_MAX_EXCERPT);
				return sessionId === null || messageId === null || kind === null || title === null || excerpt === null ? null : { sessionId, messageId, kind, title, excerpt };
			} catch {
				return null;
			}
		}
		...
		function researchNodeViewportRect(node, viewport) {
			const width = 220;
			const height = 64;
			const centerX = node.x * viewport.scale + viewport.x;
			const centerY = node.y * viewport.scale + viewport.y;
		...
		function updateResearchSelection(selection, nodeIds, mode, files) {
			...
			return { selectedNodeIds, orderedFileIds: selectedNodeIds.filter((id) => fileIds.has(id)) };
		}
		function parseResearchCanvasSelection(raw, files, artifacts) {
			...
			try {
				...
				return updateResearchSelection(EMPTY_RESEARCH_SELECTION, researchSelectionIds(value.selectedNodeIds).filter((id) => liveNodeIds.has(id)), "replace", files);
			} catch {
		...
		function researchWorkspaceSnapshot(snapshot) {
			return Object.freeze({
				files: Object.freeze(snapshot.files.map((node) => ({ ...node }))),
				artifacts: Object.freeze(snapshot.artifacts.map((node) => ({ ...node }))),
		...
		function createResearchWorkspaceSession(storage, sessionId) {
			...
			let snapshot = researchWorkspaceSnapshot({ files, artifacts, selection: loadResearchCanvasSelection(storage, sessionId, files, artifacts), viewport: { scale: 1, x: 0, y: 0 }, canvasSize: { width: 0, height: 0 }, pendingMessageJump: null });
			...
			return {
				...
				selectedFiles() { const selected = /* @__PURE__ */ new Set(snapshot.selection.orderedFileIds); return snapshot.files.filter((file) => selected.has(file.id)); },
				...
				moveNodes(selectedIds, delta, scale) { const moved = moveResearchCanvasNodes(snapshot.files, snapshot.artifacts, selectedIds, delta, scale); update(moved, true); },
				...
				cancelTransient() { update({ pendingMessageJump: null }, false); }
			};
		}
		...
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=" + JSON.stringify(tagIdResearchCanvas) + "]") === null) {
			...
			tag.textContent = cssResearchCanvas;
			document.head.appendChild(tag);
		}
		...
		function ResearchCanvas({ sessionId, t }) {
			...
			const storageRef = (0, react.useRef)(researchCanvasStorage());
			const [files, setFiles] = (0, react.useState)(() => loadResearchCanvasFiles(storageRef.current, sessionId));
			...
			(0, react.useEffect)(() => {
				...
				const ownsFileDrag = (event) => researchCanvasOwnsFileDrag(event.dataTransfer?.types);
				const onDragEnter = (event) => {
				...
				const onDrop = (event) => {
					...
					setFiles((current) => placeResearchCanvasFiles(current, dropped, point, createResearchCanvasFileId));
				};
				const onWheel = (event) => {
					event.preventDefault();
					const bounds = root.getBoundingClientRect();
					setViewport((current) => nextResearchCanvasWheel(current, {
				...
				const finishDrag = (event) => {
					...
					if (drag === null || event && event.pointerId !== drag.pointerId) return;
				...
				const onKeyDown = (event) => {
					...
					if (document.activeElement === root || root.matches(":hover")) {
						event.preventDefault();
						setSpaceVisual(true);
					}
				};
				...
				const onPointerLeave = () => {
					if (dragRef.current === null) setSpaceVisual(false);
				};
				...
				const onPointerMove = (event) => {
					...
					setViewport((current) => nextResearchCanvasPan(current, {
						deltaX,
						deltaY
					}));
				};
			...
			return (0, react_jsx_runtime.jsx)("div", {
				...
				style: {
					backgroundPosition: `${viewport.x - gridSize / 2}px ${viewport.y - gridSize / 2}px`,
				...
				children: (0, react_jsx_runtime.jsx)("div", {
					...
					children: files.map((node) => (0, react_jsx_runtime.jsx)(ResearchCanvasFileCard, { node }, node.id))
				})
			});
		}
